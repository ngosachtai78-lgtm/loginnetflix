// Bỏ qua kiểm tra SSL để fetch tới HTTPS không hợp lệ
...
app.post('/api/show-code', async (req,res)=>{
  ...
  try{
    ...
    if (/Enter Password to Access/i.test(raw)) return res.status(401).json({ ok:false, error:'Sai mật khẩu Signincode (Admin > Cài đặt)' });

    const clean = sanitizeHtml(raw, {
      allowedTags: false, disallowedTagsMode: 'discard', allowedAttributes: false, nonTextTags: ['style'],
      transformTags: { 'script':()=>({tagName:'noscript'}),'iframe':()=>({tagName:'div'}),'object':()=>({tagName:'div'}),'embed':()=>({tagName:'div'}) }
    });
